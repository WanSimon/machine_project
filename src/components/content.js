...
import {Text, View, Image, ListView, ScrollView, TouchableOpacity, ImageBackground} from 'react-native';
  ...
  updateCart(type,product){
    ...
    if(type===1){
      ...
      if(obj){
        ...
        //无可用库存
        if(product.real_stock <= product.lock_stock) return;
              ...
              {this.props.dragArr.map(item=>
                ...
                >
                  <Image
                    ...
                    source={{uri:$conf.resource_oss+item.merchant_product_info.product_info.home_thumb}}
                  />
                  <Text style={{
                    position:'absolute',
                    left:p2dWidth(170),
                  ...
                  <Text style={{
                    ...
                    {
                      this.state.cartList[item.merchant_product_info.merchant_product_id]&&this.state.cartList[item.merchant_product_info.merchant_product_id].num>0?this.state.cartList[item.merchant_product_info.merchant_product_id].num:''
                    }
                  </Text>
                  {this.state.cartList[item.merchant_product_info.merchant_product_id]&&this.state.cartList[item.merchant_product_info.merchant_product_id].num>0?<TouchableOpacity style={{
                    position:'absolute',
                    right:p2dWidth(124),
                    top:p2dHeight(185),
                    width:p2dWidth(30),
                    height:p2dWidth(30)}} onPress={()=>this.updateCart(-1,item)}>
                    <Image
                      style={{width:'100%', height:'100%'}}
                  ...
                  {
                    (item.real_stock - item.lock_stock)<1?<ImageBackground
                      style={{
                        width:'100%',
                        height:'100%'
                      }}
                      source={require('../assets/sold.png')}/>:null
                  }
