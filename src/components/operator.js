      ...
      >
        <View style={customStyle.modalContainer}>
          ...
          <View style={customStyle.modalForm}>
            ...
            <TextInput returnKeyType="go" secureTextEntry={true} password={true} placeholder='请输入维护密码' keyboardType="number-pad" onChangeText={(text) => {this.setState({txtValue: text})}} style={customStyle.textInputStyle}></TextInput>
            <Animated.View style={{ width: '100%', marginTop: 10, opacity: this.state.opacityValue }}>
              <Text style={{color: '#FF0000'}}>请输入正确的维护密码</Text>
            </Animated.View>
          </View>
          <View style={customStyle.actionContent}>
            <TouchableOpacity style={customStyle.button} onPress={() => this.cancel()}>
              <Text style={customStyle.buttonLabel}>取消</Text>
            </TouchableOpacity>
            <TouchableOpacity style={customStyle.button} onPress={() => this.confirm()}>
