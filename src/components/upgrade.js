...
class Upgrade extends Component {
  ...
  confirm() {
    if (this.state.btnDisabled.btn) {
      return;
    }
    NativeModules.RaioApi.debug(
      {msg: 'setting page upgrade components confirm func start'},
      null,
    );
        ...
        onComplete: () => {
          ...
          this.setState({
            apkData: {
              ...this.state.apkData,
              percent: 100,
              received: this.state.apkData.total,
            },
          });
          //this.setState({ btnDisabled: {...this.state.btnDisabled, btn: false} });
          //setTimeout(() => {
  ...
  async cancel() {
    try {
      ...
      NativeModules.RaioApi.debug(
        {msg: 'setting page upgrade components cancel func start'},
        null,
      );
      this.setState({btnDisabled: {...this.state.btnDisabled, btn: true}});
      let ret = await api.ignoreUpgrade({
        equipment_id: this.props.upgradeData.equipment_id,
      });
      if (ret) {
      ...
      NativeModules.RaioApi.debug(
        {
          msg: 'setting page upgrade components cancel func err',
          data: e.message || e,
        },
        null,
      );
    ...
    this.setState({btnDisabled: {...this.state.btnDisabled, btn: false}});
    NativeModules.RaioApi.debug(
      {msg: 'setting page upgrade components cancel func end'},
      null,
    );
  }
  render() {
        ...
        <View style={customStyle.modalContainer}>
          <View style={customStyle.modalContent}>
            <View style={customStyle.headerContent}>
              ...
              {this.props.upgradeData.refresh_version ? (
                <Text
                  style={{
                    fontSize: 16,
                    fontWeight: '600',
            ...
            <ScrollView style={customStyle.sectionContent}>
              {Array.isArray(this.props.upgradeData.modify_contents) &&
                this.props.upgradeData.modify_contents.map((item, idx) => (
                  <View
                    key={idx}
                    style={{
                      paddingLeft: p2dWidth(40),
                      paddingRight: p2dWidth(40),
                      paddingBottom: p2dWidth(10),
                    }}>
                    <View>
                      <Text>
                        【{item.refresh_type}】 {item.content}
                      </Text>
                    </View>
                  </View>
                ))}
            ...
            <View style={{width: '100%', marginTop: 10, opacity: 1}}>
              ...
              ) : this.state.apkData.total ? (
                <Text style={{color: '#808080', textAlign: 'center'}}>
                  {this.state.apkData.received} / {this.state.apkData.total} ({' '}
                  {this.state.apkData.percent}% )
                </Text>
              ) : (
                <Text>&nbsp;</Text>
              )}
            ...
            <View style={customStyle.actionContent}>
              {this.props.upgradeData.is_force ? null : (
                <TouchableOpacity
                  style={[
                    customStyle.button1,
                    this.state.btnDisabled.btn ? customStyle.button3 : {},
                  ]}
                  onPress={() => this.cancel()}>
