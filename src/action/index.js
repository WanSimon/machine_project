...
export function upgradeEquipmentInfo(equipmentInfo) {
  ...
  if(equipmentInfo.equipment_product_info && equipmentInfo.equipment_product_info.slot_product_list_info){
    let slotProductList = equipmentInfo.equipment_product_info.slot_product_list_info;
    for (let i = 0; i < slotProductList.length; ++i){
      let slotProductInfo = slotProductList[i];
      let bFound = false;
      for (let j = 0; j < slot_product_list_info.length; ++j){
        if (slotProductInfo.merchant_product_info.merchant_product_id === slot_product_list_info[j].merchant_product_info.merchant_product_id){
          slot_product_list_info[j].real_stock = slot_product_list_info[j].real_stock + slotProductInfo.real_stock;
          slot_product_list_info[j].lock_stock = slot_product_list_info[j].lock_stock + slotProductInfo.lock_stock;
          bFound = true;
          break;
        }
      }
