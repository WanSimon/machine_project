...
class code extends Component {
  ...
  async confirm() {
    ...
    NativeModules.RaioApi.debug(
      {msg: 'code page confirm start', method: 'code.confirm'},
      null,
    );
    try {
      ...
      // 取药码查询设备药品
      let orderInfo = await api.getEquipmentProductByCode(
        equipmentInfo.id,
        this.state.code,
      );
      ...
      if (
        orderInfo &&
        orderInfo.pick_up_product_list_info &&
        orderInfo.pick_up_product_list_info.length > 0
      ) {
        ...
        if (orderStatus != OrderStatus.OS_Paied) {
          ...
          NativeModules.RaioApi.debug(
            {
              msg: 'code page confirm err, order unpaied',
              method: 'code.confirm',
            },
            null,
          );
        ...
        for (let i = 0; i < orderInfo.pick_up_product_list_info.length; i++) {
          let merchant_product_id =
            orderInfo.pick_up_product_list_info[i].merchant_product_id;
          let product_count =
            orderInfo.pick_up_product_list_info[i].product_count -
            (orderInfo.pick_up_product_list_info[i].finished_count || 0);
          if (product_count === 0) {
            continue;
          }
          productNum += product_count;
          let dragIndex = allDrag.findIndex(
            (item) =>
          ...
          if (dragIndex === -1) {
            console.error(
              'code page confirm err, merchant_product_id:%s not found',
              merchant_product_id,
            );
            NativeModules.RaioApi.debug(
              {
                msg: `code page confirm err, merchant_product_id:${merchant_product_id} not found`,
                method: 'code.confirm',
              },
              null,
            );
          ...
          if (lock_product == LockTag.LT_Lock) {
            if (product.real_stock < product_count) {
              console.error(
                'code page confirm err, merchant_product_id:%s lack',
                merchant_product_id,
              );
              NativeModules.RaioApi.debug(
                {
                  msg: `code page confirm err, merchant_product_id:${merchant_product_id} lack`,
                  method: 'code.confirm',
                },
                null,
              );
          ...
          // 不锁库存
            ...
            if (stock < product_count) {
              console.error(
                'merchant_product_id:%s not enough',
                merchant_product_id,
              );
              NativeModules.RaioApi.debug(
                {
                  msg: `code page confirm err, merchant_product_id:${merchant_product_id} not enough`,
                  method: 'code.confirm',
                },
                null,
              );
          ...
          cartList[merchant_product_id] = {
            ...
            manufacturer:
              product.merchant_product_info.product_info.manufacturer,
        ...
        NativeModules.RaioApi.debug(
          {
            msg: 'code page confirm err, no product found',
            method: 'code.confirm',
          },
          null,
        );
    ...
    NativeModules.RaioApi.debug(
      {msg: 'home page check app version end', method: 'code.confirm'},
      null,
    );
  }
    ...
    return (
      <ImageBackground
        ...
        <View
          ...
          <View
            ...
            <View
              style={{
                ...Style.inputStyle,
                borderColor: this.state.code.length > 0 ? '#00BFCE' : '#BEBEBE',
              }}>
              <Text style={Style.codeStyle}>
                {this.state.code.length > 0 ? this.state.code[0] : ''}
              </Text>
            </View>
            <View
              style={{
                ...Style.inputStyle,
                borderColor: this.state.code.length > 1 ? '#00BFCE' : '#BEBEBE',
              }}>
              <Text style={Style.codeStyle}>
                {this.state.code.length > 1 ? this.state.code[1] : ''}
              </Text>
            </View>
            <View
              style={{
                ...Style.inputStyle,
                borderColor: this.state.code.length > 2 ? '#00BFCE' : '#BEBEBE',
              }}>
              <Text style={Style.codeStyle}>
                {this.state.code.length > 2 ? this.state.code[2] : ''}
              </Text>
            </View>
            <View
              style={{
                ...Style.inputStyle,
                borderColor: this.state.code.length > 3 ? '#00BFCE' : '#BEBEBE',
              }}>
              <Text style={Style.codeStyle}>
                {this.state.code.length > 3 ? this.state.code[3] : ''}
              </Text>
            </View>
            <View
              style={{
                ...Style.inputStyle,
                borderColor: this.state.code.length > 4 ? '#00BFCE' : '#BEBEBE',
              }}>
              <Text style={Style.codeStyle}>
                {this.state.code.length > 4 ? this.state.code[4] : ''}
              </Text>
            </View>
            <View
              style={{
                ...Style.inputStyle,
                borderColor: this.state.code.length > 5 ? '#00BFCE' : '#BEBEBE',
              }}>
              <Text style={Style.codeStyle}>
                {this.state.code.length > 5 ? this.state.code[5] : ''}
              </Text>
            </View>
          </View>
          <Text
          ...
          <TouchableOpacity
            style={{
              ...
              backgroundColor:
                this.state.code.length === 6 ? '#00BFCE' : '#ccc',
              display: 'flex',
              justifyContent: 'center',
              alignItems: 'center',
            }}
        ...
        <View
          ...
          <TouchableOpacity
            onPress={() => this.inputCode(1)}
            style={{...Style.btnStyle, borderTopLeftRadius: p2dWidth(8)}}>
            <Text style={Style.numberStyle}>1</Text>
          </TouchableOpacity>
          <TouchableOpacity
            onPress={() => this.inputCode(2)}
            style={Style.btnStyle}>
            <Text style={Style.numberStyle}>2</Text>
          </TouchableOpacity>
          <TouchableOpacity
            onPress={() => this.inputCode(3)}
            style={Style.btnStyle}>
            <Text style={Style.numberStyle}>3</Text>
          </TouchableOpacity>
          <TouchableOpacity
            onPress={() => this.inputCode('b')}
            style={{
              ...Style.btnStyle,
              borderTopRightRadius: p2dWidth(8),
          ...
          <TouchableOpacity
            onPress={() => this.inputCode('4')}
            style={Style.btnStyle}>
            <Text style={Style.numberStyle}>4</Text>
          </TouchableOpacity>
          <TouchableOpacity
            onPress={() => this.inputCode('5')}
            style={Style.btnStyle}>
            <Text style={Style.numberStyle}>5</Text>
          </TouchableOpacity>
          <TouchableOpacity
            onPress={() => this.inputCode('6')}
            style={Style.btnStyle}>
            <Text style={Style.numberStyle}>6</Text>
          </TouchableOpacity>
          <TouchableOpacity
            onPress={() => this.inputCode('0')}
            style={Style.btnStyle}>
            <Text style={Style.numberStyle}>0</Text>
          </TouchableOpacity>
          <TouchableOpacity
            onPress={() => this.inputCode('7')}
            style={{...Style.btnStyle, borderBottomLeftRadius: p2dWidth(40)}}>
            <Text style={Style.numberStyle}>7</Text>
          </TouchableOpacity>
          <TouchableOpacity
            onPress={() => this.inputCode('8')}
            style={Style.btnStyle}>
            <Text style={Style.numberStyle}>8</Text>
          </TouchableOpacity>
          <TouchableOpacity
            onPress={() => this.inputCode('9')}
            style={Style.btnStyle}>
            <Text style={Style.numberStyle}>9</Text>
          </TouchableOpacity>
          <TouchableOpacity
            onPress={() => this.inputCode('c')}
            style={{
              ...Style.btnStyle,
              borderBottomRightRadius: p2dWidth(40),
              backgroundColor: '#E1E1E1',
