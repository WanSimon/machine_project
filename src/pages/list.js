...
import {BuyWay, OrderSource, OrderStatus, PayStatus, PickUpType} from "../js/common";
...
class list extends Component {
  ...
  async componentDidMount(){
    ...
    let allDrag = equipmentInfo.product_list
      .sort((a,b)=>{return ((b.real_stock- (b.lock_stock||0)) - (a.real_stock - (a.lock_stock||0)))});
  ...
  setCartList(cartList){
    let productNum = 0,totalPrice = 0,totalCustomerPrice = 0 ;
    ...
    this.setState({cartList:cartList,productNum,totalPrice,totalCustomerPrice});
    let action = upgradeCart({cartList:cartList,productNum,totalPrice,totalCustomerPrice});
    ...
    if(searchText!==this.state.searchText){
      ...
      if(searchText===""){
        ...
        let dragArr = this.state.allDrag
          .filter(drag=>drag.merchant_product_info.product_info.name.indexOf(searchText)!==-1)
          .sort((a,b)=>{return ((b.real_stock- (b.lock_stock||0)) - (a.real_stock - (a.lock_stock||0)))});
  ...
  setType(type){
    ...
    let productList = this.state.allDrag.filter(item => {
      let r1 = true, r2 = true;
      if(this.state.searchText) r1 = (-1 !== item.merchant_product_info.product_info.name.indexOf(this.state.searchText));
      //非全部
      if('1' !== type){
        r2 = false;
  ...
  generateOrder(){
    ...
    console.info(`list page first generate order_id=${order_id}, trade_no=${trade_no}`);
    NativeModules.RaioApi.debug({msg: `list page first generate order_id=${order_id}, trade_no=${trade_no}`, method: 'list.generateOrder'}, null);
    ...
    NativeModules.RaioApi.debug({msg: `list page upgrade reducex orderInfo = ${JSON.stringify(orderInfo)}`, method: 'list.generateOrder'}, null);
  ...
  render() {
    return (
      <View store={store} style={{
        ...
        {
          this.state.cartVisible?<Cart cartList={this.state.cartList} setCartList = {this.setCartList.bind(this)} setCartVisible={this.setCartVisible.bind(this)}/>:null
        }
      </View>
    );
  }
...
export default connect(state=>({cart:state.cart,equipmentInfo:state.equipmentInfo}))(list);
