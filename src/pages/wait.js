...
import {upgradeEquipmentInfo, upgradePickupStatus, upgradeFailedProduct} from '../action';
...
class wait extends Component {
  ...
  async componentDidMount(){
    ...
    this.emitListener = DeviceEventEmitter.addListener('out_callback', (res) => {
      ...
      NativeModules.RaioApi.debug({msg: `wait page, listen out_callback res = ${JSON.stringify(res)}`, method: 'wait.componentDidMount'}, null); 
    ...
    NativeModules.RaioApi.debug({msg: `wait page componentDidMount state orderInfo = ${JSON.stringify(orderInfo)}`, method: 'wait.componentDidMount'}, null); 
  ...
  async pickup(drugArr){
    ...
    NativeModules.RaioApi.debug({msg: `wait page func pickup get state orderInfo = ${orderInfo}`, method: 'wait.pickup'}, null); 
    //TODO log
        ...
        if(slot.real_stock > slot.lock_stock){
          ...
          if(equipment_slot_obj && equipment_slot_obj.length>0){
            ...
            try{
              ...
              NativeModules.RaioApi.debug({msg: `wait page func out err = ${e.message}`, method: 'wait.pickup'}, null); 
              // todo 取药失败
              result =false;
              drugArr[i].status = 3;
  ...
  async print_success() {
    try {
      ...
      let orderInfo = store.getState().orderInfo;
      console.info(`wait page func print_success get state orderInfo = %o`, orderInfo);
      NativeModules.RaioApi.debug({msg: `wait page func print_success get state orderInfo = ${orderInfo}`, method: 'wait.print_success'}, null); 
      if (!orderInfo.serial_no) {
        let res = await api.getOrderInfo(orderInfo.id);
  ...
  async print_fail(drugArr){
    ...
    try {
      ...
      let customerFlag = store.getState().customerFlag;
      let orderInfo = store.getState().orderInfo;
      console.info(`wait page func print_fail get state orderInfo = %o`, orderInfo);
      NativeModules.RaioApi.debug({msg: `wait page func print_fail get state orderInfo = ${orderInfo}`, method: 'wait.print_fail'}, null); 
      //TODO log
      if (!orderInfo.serial_no) {
        let res = await api.getOrderInfo(orderInfo.id);
        orderInfo.serial_no = res.serial_no || " ";
  ...
  //出药
  out(x,y){
    NativeModules.RaioApi.debug({msg: `call out input x=${x},y=${y}`, method: 'wait.out'}, null); 
    return new Promise((resolve, reject)=>{
      if(Conf.debug){
        resolve();
      } else {
