...
class codeWait extends Component {
  ...
  async componentDidMount() {
    ...
    this.emitListener = DeviceEventEmitter.addListener(
      ...
      (res) => {
        ...
        NativeModules.RaioApi.debug(
          {
            msg: `codeWait page, listen out_callback res = ${JSON.stringify(
              res,
            )}`,
            method: 'codeWait.componentDidMount',
          },
          null,
        );
    ...
    console.info(
      'codeWait page componentDidMount computed drugArr = %o',
      drugArr,
    );
    NativeModules.RaioApi.debug(
      {
        msg: `codeWait page componentDidMount computed drugArr = ${JSON.stringify(
          drugArr,
        )}`,
        method: 'codeWait.componentDidMount',
      },
    ...
    console.info(
      'codeWait page componentDidMount state orderInfo = %o',
      orderInfo,
    );
    NativeModules.RaioApi.debug(
      {
        msg: `codeWait page componentDidMount state orderInfo = ${JSON.stringify(
          orderInfo,
        )}`,
        method: 'codeWait.componentDidMount',
      },
    ...
    if (!orderInfo.serial_no) {
      try {
        ...
        console.info(
          'codeWait page componentDidMount getOrderInfo.serial_no err = %o',
          e,
        );
        NativeModules.RaioApi.debug(
          {
            msg: `codeWait page componentDidMount getOrderInfo.serial_no err = ${e.message}`,
            method: 'codeWait.componentDidMount',
          },
          null,
        );
    ...
    try {
      ...
      NativeModules.RaioApi.debug(
        {
          msg: `codeWait page componentDidMount call pickup err = ${e.message}`,
          method: 'codeWait.componentDidMount',
        },
        null,
      );
  ...
  async pickup(drugArr) {
    ...
    let orderInfo = store.getState().codeOrder.orderInfo;
    console.info(
      'codeWait page func pickup get state orderInfo = %o',
      orderInfo,
    );
    NativeModules.RaioApi.debug(
      {
        msg: `codeWait page func pickup get state orderInfo = ${orderInfo}`,
        method: 'codeWait.pickup',
      },
      null,
    );
    ...
    for (let i = 0; i < drugArr.length; i++) {
      ...
      // 排序
      product_slot_list = product_slot_list.sort((a, b) => {
        return (
          Math.min(b.real_stock, b.lock_stock) -
          Math.min(a.real_stock, a.lock_stock)
        );
      });
            ...
            try {
              ...
              if (orderInfo.lock_product == LockTag.LT_Lock) {
                if (slot.lock_stock > 0) {
                  ...
                  console.error(
                    'exception lock_stock:%d,equipment_id:%s,slot_no:%d',
                    slot.lock_stock,
                    equipmentInfo.id,
                    slot_no,
                  );
              ...
              NativeModules.RaioApi.debug(
                {
                  msg: `codeWait page func out err = ${e.message}`,
                  method: 'codeWait.pickup',
                },
                null,
              );
  ...
  async print_success() {
    try {
      ...
      console.info(
        'codeWait page func print_success get state orderInfo = %o',
        orderInfo,
      );
      NativeModules.RaioApi.debug(
        {
          msg: `codeWait page func print_success get state orderInfo = ${orderInfo}`,
          method: 'codeWait.pickup',
        },
        null,
      );
  ...
  async print_fail(drugArr) {
    ...
    try {
      ...
      NativeModules.RaioApi.debug(
        {
          msg: `codeWait page func print_fail get state orderInfo = ${orderInfo}`,
          method: 'codeWait.print_fail',
        },
        null,
      );
  ...
  out(x, y) {
    NativeModules.RaioApi.debug(
      {msg: `call out input x=${x},y=${y}`, method: 'codeWait.out'},
      null,
    );
    return new Promise((resolve, reject) => {
