...
import {OrderStatus,PayType,PayStatus,BuyWay,OrderSource,PickUpType,TradeType} from '../js/common'
...
class order extends Component {
  ...
  async componentDidMount(){
    ...
    NativeModules.RaioApi.debug({msg: `order page get reducex orderInfo = ${JSON.stringify(orderInfo)}`, method: 'order.componentDidMount'}, null);
    ...
    NativeModules.RaioApi.debug({msg: `order page rebuild orderInfo = ${JSON.stringify(orderInfo)}`, method: 'order.componentDidMount'}, null);
    ...
    this.didBlurListener = this.props.navigation.addListener(
      'didBlur',
      () => {
        if(this.checkCustomerTimer) clearTimeout(this.checkCustomerTimer);
      });
  ...
  async submitOrder(orderInfo){
    NativeModules.RaioApi.debug({msg: 'order page submitOrder start', method: 'order.submitOrder'}, null);
    try{
      let res = await api.submitOrder(orderInfo);
      NativeModules.RaioApi.debug({msg: `order page submitOrder success, ${JSON.stringify(res)}`, method: 'order.submitOrder'}, null);
    }catch (e) {
      console.error(e);
      NativeModules.RaioApi.debug({msg: `order page submitOrder fail, ${e.message}`, method: 'order.submitOrder'}, null);
    }
    NativeModules.RaioApi.debug({msg: 'order page submitOrder end', method: 'order.submitOrder'}, null);
  }

  async checkCustomer(order_id){
    let orderInfo;
    NativeModules.RaioApi.debug({msg: 'order page checkCustomer start', method: 'order.checkCustomer'}, null);
    try{
      orderInfo = await api.getOrderInfo(order_id);
      NativeModules.RaioApi.debug({msg: `order page checkCustomer getOrderInfo success, ${JSON.stringify(orderInfo)}`, method: 'order.checkCustomer'}, null);
    }catch (e) {
      console.error(e);
      NativeModules.RaioApi.debug({msg: `order page checkCustomer getOrderInfo fail, ${e.message}`, method: 'order.checkCustomer'}, null);
    }
    //更新交易流水号
    let order = store.getState().orderInfo;
    NativeModules.RaioApi.debug({msg: `order page checkCustomer get orderInfo from reducex, ${JSON.stringify(order)}`, method: 'order.checkCustomer'}, null);
    ...
    NativeModules.RaioApi.debug({msg: 'home page checkCustomer end', method: 'order.checkCustomer'}, null);
  }
  ...
  async pay(trade_no){
    ...
    payInfo.trade_typ = this.state.payMode ==='ali'?TradeType.ALI_NATIVE:TradeType.WX_NATIVE;
  ...
  render() {
    return (
      <View style={{
        ...
        <View style={{marginTop:p2dHeight(20),backgroundColor:'#fff',flexGrow: 1}}>
          <View>
            <Text style={{
              height:p2dHeight(147),
              lineHeight:p2dHeight(147),
          ...
          <TouchableOpacity
            ...
            <ImageBackground
              ...
              source={this.state.payMode==='ali'?require('../assets/aliPay.png'):require('../assets/ali.png')}/>
          </TouchableOpacity>
          <TouchableOpacity
            style={{
              position:'absolute',
            ...
            <ImageBackground
              ...
              imageStyle={{resizeMode:'contain'}}
              source={this.state.payMode==='wechat'?require('../assets/wechatPay.png'):require('../assets/wechat.png')}/>
          </TouchableOpacity>
          <View
            style={{
              position:'absolute',
        ...
        {this.state.loginVisible?<Login qrcode={`http://c.cinyou.com/?o=${this.state.order_id}`} hideLogin={this.hideLogin.bind(this)}/>:null}
      </View>
    );
  }
}
